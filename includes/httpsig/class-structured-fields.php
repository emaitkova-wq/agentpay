...
class StructuredFields {
    ...
    public static function parseSignatureInput($input) {
        ...
        while ($pos < $len) {
            ...
            if ($label === null) {
                throw new \InvalidArgumentException("Expected dictionary key at position {$pos}");
            }
            ...
            $value_start = $pos;
            if ($pos >= $len || $input[$pos] !== '(') {
                throw new \InvalidArgumentException("Expected '(' for inner list at position {$pos}");
            }
            ...
            if ($pos < $len && $input[$pos] === ',') {
                ...
                throw new \InvalidArgumentException("Unexpected character '{$input[$pos]}' at position {$pos}");
            }
        }
        return $result;
    }
    ...
    public static function parseSignature($input) {
        ...
        while ($pos < $len) {
            ...
            $pos++;
            if ($pos >= $len || $input[$pos] !== ':') {
                throw new \InvalidArgumentException("Expected ':' for byte sequence at position {$pos}");
            }
            $pos++;
            $end = strpos($input, ':', $pos);
            if ($end === false) {
                throw new \InvalidArgumentException("Unterminated byte sequence");
            }
            $b64 = substr($input, $pos, $end - $pos);
            $bytes = base64_decode($b64, true);
            if ($bytes === false) {
                throw new \InvalidArgumentException("Invalid base64 in signature value");
            }
            $pos = $end + 1;
            $params = self::parseParameters($input, $pos);
    ...
    public static function parseDictionary($input) {
        ...
        while ($pos < $len) {
            ...
            if ($key === null) {
                throw new \InvalidArgumentException("Expected key at position {$pos}");
            }
            $value = true;
            if ($pos < $len && $input[$pos] === '=') {
                $pos++;
    ...
    private static function parseInnerListItems($input, &$pos) {
        ...
        while ($pos < $len && $input[$pos] !== ')') {
            if ($input[$pos] !== '"') {
                throw new \InvalidArgumentException("Expected quoted string at position {$pos}");
            }
            $pos++;
            $name = '';
            while ($pos < $len && $input[$pos] !== '"') {
                if ($input[$pos] === '\\' && $pos + 1 < $len) {
                    ...
                    throw new \InvalidArgumentException("Invalid escape sequence");
                }
                $name .= $input[$pos];
                $pos++;
            }
        ...
        if ($pos >= $len) {
            throw new \InvalidArgumentException("Unterminated inner list");
        }
        $pos++;
        return $items;
    }
    ...
    private static function parseBareItem($input, &$pos) {
        $len = strlen($input);
        if ($pos >= $len) { throw new \InvalidArgumentException("Unexpected end"); }
        $c = $input[$pos];
        ...
        if ($c === '"') {
            ...
            if ($pos >= $len) { throw new \InvalidArgumentException("Unterminated string"); }
            $pos++;
            return $val;
        }
        if ($c === ':') {
            $pos++;
            $end = strpos($input, ':', $pos);
            if ($end === false) { throw new \InvalidArgumentException("Unterminated byte sequence"); }
            $val = base64_decode(substr($input, $pos, $end - $pos), true);
            if ($val === false) { throw new \InvalidArgumentException("Invalid base64"); }
            $pos = $end + 1;
            return $val;
        }
        if ($c === '?') {
        ...
        throw new \InvalidArgumentException("Unexpected character '{$c}' at position {$pos}");
    }
